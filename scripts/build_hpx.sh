...
if [[ "${CXX}" == "clang++" ]]; then # clang too old on our usual machine - compile without CUDA
  cmake -DCMAKE_CXX_COMPILER=${HPX_COMPILER} -DCMAKE_EXPORT_COMPILE_COMMANDS=ON -DCMAKE_BUILD_TYPE=${CMAKE_BUILD_TYPE} -DHPX_WITH_EXAMPLES=OFF -DHPX_WITH_TESTS=OFF -DHPX_WITH_CXX17=ON -DHPX_WITH_CUDA=ON -DHPX_WITH_MALLOC=${HPX_MALLOC} -DCMAKE_INSTALL_PREFIX=${INSTALL_DIR} ${SOURCE_DIR} 
else
  cmake -DCMAKE_CXX_COMPILER=${HPX_COMPILER} -DCMAKE_EXPORT_COMPILE_COMMANDS=ON -DCMAKE_BUILD_TYPE=${CMAKE_BUILD_TYPE} -DHPX_WITH_EXAMPLES=OFF -DHPX_WITH_TESTS=OFF -DHPX_WITH_CXX17=ON -DHPX_WITH_CUDA=ON -DHPX_WITH_MALLOC=${HPX_MALLOC} -DCMAKE_INSTALL_PREFIX=${INSTALL_DIR} ${SOURCE_DIR} 
fi
